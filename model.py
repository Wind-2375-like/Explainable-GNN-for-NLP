...
from GCN import RGCNConv, GraphConv, GCNConv
...
def batch_graphify(features, qmask, lengths, window_past, window_future, edge_type_mapping, att_model, no_cuda):
    ...
    # node_features: (sum(each_example_actual_seq_len)->total node nums, 2*D_e)
    # edge_index: (2, total edge nums), edge_index[0]: start node nums, edge_index[1]: end node nums
    # edge_norm: (total edge nums,): attention score of each edge.
...
class GraphNetwork(torch.nn.Module):
    ...
    def get_vertex_embedding_dims(self):
        return np.array([self.conv1.in_channels, self.conv2.in_channels])

    def get_message_dims(self):
        return np.array([self.conv1.out_channels, self.conv2.out_channels])

    def get_latest_source_embeddings(self):
        return [self.conv1.get_latest_source_embeddings(), self.conv2.get_latest_source_embeddings()]

    def get_latest_target_embeddings(self):
        return [self.conv1.get_latest_target_embeddings(), self.conv2.get_latest_target_embeddings()]

    def get_latest_messages(self):
        return [self.conv1.get_latest_messages(), self.conv2.get_latest_messages()]
    ...
    def forward(self, x, edge_index, edge_norm, edge_type, seq_lengths, umask, nodal_attn, avec):
        ...
        if self.injected_message_scale is not None:
            message_scale = self.injected_message_scale[0]
        else:
            message_scale = None
        if self.injected_message_replacement is not None:
            message_replacement = self.injected_message_replacement[0][0]
        else:
            message_replacement = None
        out = self.conv1(x, edge_index, edge_type, edge_norm=edge_norm, message_scale=message_scale, message_replacement=message_replacement)
        # GraphConv eq.3
        if self.injected_message_scale is not None:
            message_scale = self.injected_message_scale[1]
        else:
            message_scale = None
        if self.injected_message_replacement is not None:
            message_replacement = self.injected_message_replacement[0][1]
        else:
            message_replacement = None
        out = self.conv2(out, edge_index, message_scale=message_scale, message_replacement=message_replacement)
        # concat(gi, hi(2)) eq.4
...
class DialogueGCNModel(nn.Module):
    ...
    def forward(self, U, qmask, umask, seq_lengths):
        ...
        if self.base_model == "DialogRNN":
            ...
            # (seq_len, batch, D_m) -> (seq_len, batch, 2 * D_e)
            emotions, hidden = self.lstm(U)
...
class TextGCN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        if self.injected_message_replacement is not None:
            ...
            message_replacement = None
        x = F.relu(self.conv1(x, edge_index, edge_weight, message_scale=message_scale, message_replacement=message_replacement))
        ...
        if self.injected_message_replacement is not None:
            ...
            message_replacement = None
        x = self.conv2(x, edge_index, edge_weight, message_scale=message_scale, message_replacement=message_replacement)
